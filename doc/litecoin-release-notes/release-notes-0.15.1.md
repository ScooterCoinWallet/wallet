Scootercoin Core version *0.15.1* is now available from:

  <https://download.scootercoin.org/scootercoin-0.15.1/>
...
A number of changes to the way Scootercoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 miniscoot per kB (0.00001 SCOOT). This is to prepare for dropping the minimum transaction fee to 0.1 miniscoot per kB (0.0001 SCOOT) in the next release.
