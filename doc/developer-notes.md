...
- [Development guidelines](#development-guidelines)
    - [General Scootercoin Core](#general-scootercoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Run with the `-testnet` option to run with "play scootercoins" on the test network, if you
...
([`valgrind.supp`](https://github.com/scootercoin-project/scootercoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_scootercoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_scootercoin --log_level=test_suite
...
# A coverage report will now be accessible at `./test_scootercoin.coverage/index.html`.
```
...
- ThreadSocketHandler : Sends/Receives data from peers on port 25333.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 25332 for connections and services them.
